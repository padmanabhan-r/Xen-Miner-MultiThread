...
def mine_hash(args):
    target_substr, prev_hash, memory_cost, difficulty, cores = args
    ...
    if target_substr in hashed_data[-87:]:
        return hashed_data, random_data
    return None, None

def mine_block(target_substr, prev_hash, pbar, pool):
    global memory_cost 
    attempts = 0
    random_data = None
    ...
    while True:
        ...
        tasks = [(target_substr, prev_hash, memory_cost, difficulty, 1) for _ in range(cpu_count())]
        results = pool.map(mine_hash, tasks)
        for hashed_data, random_data in results:
            if hashed_data:
                end_time = time.time()
...
def verify_block(block):
    argon2_hasher = argon2.using(time_cost=difficulty, memory_cost=memory_cost, parallelism=1)
        ...
        with Pool(processes=cpu_count()) as pool:
            with tqdm(total=None, dynamic_ncols=True, desc="Mining", unit="hash") as pbar:
                for i in range(1, num_blocks_to_mine + 1):
                    random_data, new_valid_hash, attempts, hashes_per_second = mine_block(target_substr, blockchain[-1]['hash'], pbar, pool)
